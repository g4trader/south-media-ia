#!/usr/bin/env python3
"""
Gerar dashboard final sem referências ao Netflix
"""
...
def generate_dashboard_no_netflix():
    """Gerar dashboard sem Netflix"""
    
    print("🔄 GERANDO DASHBOARD SEM NETFLIX")
    print("=" * 70)
    
    # Carregar template sem Netflix
    template_files = glob.glob("templates/template_simple_no_netflix_*.html")
    if not template_files:
        print("❌ Nenhum template sem Netflix encontrado")
        return
    ...
    dashboard_filename = f"static/dash_semana_do_pescado_NO_NETFLIX_{timestamp}.html"
    
    with open(dashboard_filename, 'w', encoding='utf-8') as f:
        f.write(template_content)
    
    print(f"\n✅ DASHBOARD SEM NETFLIX SALVO: {dashboard_filename}")
